...
Library.DirtyGroupboxes = {};
Library.DirtyDependencyBoxes = false;
...
function Library:ResumeLayout()
    Library.LayoutSuspended = false
    for groupbox in pairs(Library.DirtyGroupboxes) do
        if type(groupbox.Resize) == 'function' then
            pcall(groupbox.Resize, groupbox)
        end
    end
    table.clear(Library.DirtyGroupboxes)
    if Library.DirtyDependencyBoxes or type(Library.UpdateDependencyBoxes) == 'function' then
        Library.DirtyDependencyBoxes = false
        pcall(Library.UpdateDependencyBoxes, Library)
    end
    local win = Window or Library.Window
    if win and win.Tabs then
        for _, tab in pairs(win.Tabs) do
            if type(tab) == 'table' then
                ...
                for _, tabbox in pairs(tab.Tabboxes or {}) do
                    if type(tabbox.Resize) == 'function' then
                        pcall(tabbox.Resize, tabbox)
                    else
                        for _, subtab in pairs(tabbox.Tabs or {}) do
                            if subtab.Container and subtab.Container.Visible and type(subtab.Resize) == 'function' then
                                pcall(subtab.Resize, subtab)
                            end
                        end
                    end
                end
...
function Library:UpdateDependencyBoxes()
    if Library.LayoutSuspended or Library.ConfigLoading then
        Library.DirtyDependencyBoxes = true
        return
    end
...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        local ColorPicker = {
            ...
            Owner = self;
            Container = self.Container or (self.Parent and self.Parent.Container);
        };
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        if ParentObj.RefreshToggleAddonLayout then
            task.defer(ParentObj.RefreshToggleAddonLayout)
        end
...
do
    ...
    function Funcs:AddRow(Columns)
        ...
        for i = 1, ColumnsCount do
            ...
            function Box:Resize()
                if Library.LayoutSuspended then return end
                local maxHeight = 0
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local function RefreshToggleAddonLayout()
            local textSize = ToggleLabel.TextSize or Library.FontSize or 14
            local textW = 0
            local okBounds, bx = pcall(function()
                return Library:GetTextBounds(ToggleLabel.Text, Library.Font, textSize)
            end)
            if okBounds and type(bx) == 'number' then
                textW = bx
            else
                textW = math.max(#tostring(ToggleLabel.Text) * 7, 24)
            end
            local addonW = 0
            local addonN = 0
            for _, child in ipairs(ToggleLabel:GetChildren()) do
                if child:IsA('GuiObject') then
                    addonN = addonN + 1
                    addonW = addonW + math.max(child.AbsoluteSize.X, 28)
                end
            end
            local pad = (addonN > 0) and (5 * addonN + 8) or 4
            ToggleLabel.Size = UDim2.new(0, math.ceil(textW + pad + addonW), 1, 0)
        end
        Toggle.RefreshToggleAddonLayout = RefreshToggleAddonLayout
        ToggleLabel:GetPropertyChangedSignal('Text'):Connect(RefreshToggleAddonLayout)
        ToggleLabel.ChildAdded:Connect(function()
            task.defer(RefreshToggleAddonLayout)
        end)
        task.defer(RefreshToggleAddonLayout)
        ...
        function Toggle:SetValue(Bool)
            ...
            if not Library.ConfigLoading and not Library.LayoutSuspended then
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        function Slider:Display()
            ...
            local alpha = (span == 0) and 1 or math.clamp((Slider.Value - Slider.Min) / span, 0, 1)
            Fill.Size = UDim2.new(alpha, 0, 1, 0);

            HideBorderRight.Visible = not (alpha >= 0.999 or alpha <= 0);
        end;
        ...
        SliderInner.InputBegan:Connect(function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and not Library:MouseIsOverOpenedFrame() then
                ...
                local function UpdateSlider(PosX)
                    ...
                    local nX = math.clamp(Diff, 0, math.max(SliderInner.AbsoluteSize.X, 1))
    ...
    function Funcs:AddDependencyBox()
        ...
        function Depbox:Resize()
            if Library.LayoutSuspended then
                return
            end
            local depH = Layout.AbsoluteContentSize.Y
            if depH <= 0 then
                local elemSum = 0
                for _, elem in next, Frame:GetChildren() do
                    if (not elem:IsA('UIListLayout')) and elem.Visible then
                        elemSum = elemSum + elem.Size.Y.Offset
                    end
                end
                if elemSum > 0 then
                    depH = elemSum
            ...
            Holder.Size = UDim2.new(1, 0, 0, depH);
            Groupbox:Resize();
        end;
...
function Library:CreateWindow(...)
    ...
    function Window:AddTab(Name)
        ...
        function Tab:ShowTab()
            ...
            for _, Side in next, { LeftSide, RightSide } do
                if Side then
                    local layout = Side:FindFirstChildWhichIsA('UIListLayout');
                    if layout and type(layout.ApplyLayout) == 'function' then
                        pcall(layout.ApplyLayout, layout);
                    end;
                    if layout then
        ...
        function Tab:AddGroupbox(Info)
            ...
            function Groupbox:Resize()
                if Library.LayoutSuspended then
                    Library.DirtyGroupboxes[Groupbox] = true
                    return
                end
                local Size = 0;
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:Resize()
                local active = Tabbox.SelectedTab
                if not active then
                    for _, t in next, Tabbox.Tabs do
                        if t.Container and t.Container.Visible then
                            active = t
                            break
                        end
                    end
                ...
                if not active then
                    active = next(Tabbox.Tabs)
                end
                if active and type(active.Resize) == 'function' then
                    active:Resize()
                end
            end;
            Library:AddToRegistry(BoxOuter, {
            ...
            local TabboxGrid = Library:Create('UIGridLayout', {
                FillDirection = Enum.FillDirection.Horizontal;
                HorizontalAlignment = Enum.HorizontalAlignment.Left;
                VerticalAlignment = Enum.VerticalAlignment.Top;
                SortOrder = Enum.SortOrder.LayoutOrder;
                CellPadding = UDim2.fromOffset(1, 1);
                CellSize = UDim2.new(0.5, -1, 0, 17);
                FillDirectionMaxCells = 4;
                Parent = TabboxButtons;
            });
            Tabbox.Grid = TabboxGrid;
            Tabbox.Buttons = TabboxButtons;
            function Tabbox:AddTab(Name)
                local Tab = {};
                local Button = Library:Create('Frame', {
                    BackgroundColor3 = Library.MainColor;
                    BorderColor3 = Color3.new(0, 0, 0);
                    Size = UDim2.new(0.5, 0, 0, 17);
                    ZIndex = 6;
                    ClipsDescendants = true;
                    Parent = TabboxButtons;
                });
                ...
                local ButtonLabel = Library:CreateLabel({
                    Size = UDim2.new(1, -4, 1, 0);
                    Position = UDim2.new(0, 2, 0, 0);
                    TextSize = math.max((Library.FontSize or 14) - 1, 12);
                    Text = Name;
                    TextXAlignment = Enum.TextXAlignment.Center;
                ...
                function Tab:Resize()
                    if Library.LayoutSuspended then
                        Library.DirtyGroupboxes[Tab] = true
                        return
                    end
                    local TabCount = 0;
                    ...
                    local columns = TabCount;
                    local rows = 1;
                    if TabCount >= 5 then
                        columns = math.ceil(TabCount / 2);
                        rows = 2;
                    end;

                    local cellH = 17;
                    local headerH = (cellH * rows) + math.max(rows - 1, 0) + 2;
                    TabboxButtons.Size = UDim2.new(1, 0, 0, headerH);
                    if Tabbox.Grid then
                        Tabbox.Grid.FillDirectionMaxCells = columns;
                        Tabbox.Grid.CellSize = UDim2.new(1 / columns, -1, 0, cellH);
                    end

                    for _, other in next, Tabbox.Tabs do
                        if other.Container then
                            other.Container.Position = UDim2.new(0, 4, 0, headerH);
                            other.Container.Size = UDim2.new(1, -4, 1, -headerH);
                        end
                    end

                    if (not Container.Visible) and Tabbox.SelectedTab ~= Tab then
                        return;
                    end;
